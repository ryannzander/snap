...
Unit tests live in `ios/SnapTests` and run with the `Snap` scheme. `ios/SnapUITests` proves on a real HealthKit store that a simulated workout is not flagged hand-entered; it drives a system permission sheet, so exclude it from routine runs with `-skip-testing:SnapUITests` and run it deliberately with `-only-testing:SnapUITests`.
